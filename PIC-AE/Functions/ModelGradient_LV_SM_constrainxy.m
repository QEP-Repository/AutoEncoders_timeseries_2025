...
function [gradient,MSE,Physics,MSE_xy] = ModelGradient_LV_SM_constrainxy(parameters,AE,dlX_0,dlX_1,Noise,alpha,Model,dlX_0b,dlx_b,dly_b)
    ...
    [X0_p,Code0] = AE_network_LV_SM(dlX_0b,[],1,parameters,AE,Noise);

    MSE_xy = mean((Code0(1,:)-dlx_b).^2 + (Code0(2,:)-dly_b).^2);
        
    %% Total Loss and Graidents

    Loss = (MSE + alpha.*Physics + alpha.*MSE_xy)./(1+alpha);
